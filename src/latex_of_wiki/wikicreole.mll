...
let from_channel (type param) (type flow) ?(sectioning = false) param builder ch =
  let module B = (val builder : Builder with type param = param and type flow = flow) in
  let module Wikicreole = Make(B) in
  Lwt_io.read ch >|= fun contents ->
  Wikicreole.from_lexbuf_no_preempt sectioning param (Lexing.from_string contents)
